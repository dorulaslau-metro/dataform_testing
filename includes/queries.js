...
SELECT *,
CONCAT(
    FORMAT_DATE('%d/%m', DATE_TRUNC(Date, WEEK(MONDAY))), ' - ',
    FORMAT_DATE('%d/%m', DATE_TRUNC(Date, WEEK(MONDAY)) + INTERVAL 6 DAY),
    ' ', FORMAT_DATE('%Y', Date)
  ) AS year_week
FROM
(
(SELECT
  Store_name, Country, Date, Store_no, Tool, Tool_new, Department_name, Department_no, PMMG, PMG, PMSG, User_type,
  CASE WHEN Tool = 'bbd_to_be_checked' OR Tool = 'bbd_missing' THEN COUNT(PK) ELSE 0 END AS notification_count,
  CASE WHEN Tool IN ('bbd_checked_not_found', 'bbd_checked_removed_from_shelf', 'bbd_added') OR Tool LIKE 'bbd_actioned_%' THEN COUNT(DISTINCT PK4special) ELSE 0 END AS work_count,
 
  COUNT(DISTINCT PK2)                                       AS bbd_count,
  COUNT(DISTINCT PK3)                                       AS article_count,
  ...
  ----------
  CASE WHEN Tool = 'bbd_checked_not_found' AND Reaction_time = 'Slow actioned' THEN COUNT(PK) ELSE 0 END AS bbdCheckedNotFound_SlowActioned,
  CASE WHEN Tool = 'bbd_checked_not_found' AND Reaction_time = 'Medium actioned' THEN COUNT(PK) ELSE 0 END AS bbdCheckedNotFound_MediumActioned,
  CASE WHEN Tool = 'bbd_checked_not_found' AND Reaction_time = 'Fast actioned' THEN COUNT(PK) ELSE 0 END AS bbdCheckedNotFound_FastActioned,
  CASE WHEN Tool = 'bbd_checked_not_found' AND Reaction_time = 'Expired-no action' THEN COUNT(PK) ELSE 0 END AS bbdCheckedNotFound_ExpiredNoAction,
  CASE WHEN Tool = 'bbd_checked_removed_from_shelf' AND Reaction_time = 'Slow actioned' THEN COUNT(PK) ELSE 0 END AS bbdCheckedRemovedFromShelf_SlowActioned,
  CASE WHEN Tool = 'bbd_checked_removed_from_shelf' AND Reaction_time = 'Medium actioned' THEN COUNT(PK) ELSE 0 END AS bbdCheckedRemovedFromShelf_MediumActioned,
  CASE WHEN Tool = 'bbd_checked_removed_from_shelf' AND Reaction_time = 'Fast actioned' THEN COUNT(PK) ELSE 0 END AS bbdCheckedRemovedFromShelf_FastActioned,
  CASE WHEN Tool = 'bbd_checked_removed_from_shelf' AND Reaction_time = 'Expired-no action' THEN COUNT(PK) ELSE 0 END AS bbdCheckedRemovedFromShelf_ExpiredNoAction,
  CASE WHEN Tool LIKE 'bbd_checked_%' AND Reaction_time = 'Slow actioned' THEN COUNT(PK) ELSE 0 END AS bbdChecked_SlowActioned,
  CASE WHEN Tool LIKE 'bbd_checked_%' AND Reaction_time = 'Medium actioned' THEN COUNT(PK) ELSE 0 END AS bbdChecked_MediumActioned,
  CASE WHEN Tool LIKE 'bbd_checked_%' AND Reaction_time = 'Fast actioned' THEN COUNT(PK) ELSE 0 END AS bbdChecked_FastActioned,
  CASE WHEN Tool LIKE 'bbd_checked_%' AND Reaction_time = 'Expired-no action' THEN COUNT(PK) ELSE 0 END AS bbdChecked_ExpiredNoAction,
  CASE WHEN Tool LIKE 'bbd_actioned_%' AND Reaction_time = 'Slow actioned' THEN COUNT(DISTINCT PK4special) ELSE 0 END AS bbdActioned_SlowActioned,
  CASE WHEN Tool LIKE 'bbd_actioned_%' AND Reaction_time = 'Medium actioned' THEN COUNT(DISTINCT PK4special) ELSE 0 END AS bbdActioned_MediumActioned,
  CASE WHEN Tool LIKE 'bbd_actioned_%' AND Reaction_time = 'Fast actioned' THEN COUNT(DISTINCT PK4special) ELSE 0 END AS bbdActioned_FastActioned,
  CASE WHEN Tool LIKE 'bbd_actioned_%' AND Reaction_time = 'Expired-no action' THEN COUNT(DISTINCT PK4special) ELSE 0 END AS bbdActioned_ExpiredNoAction,
 
  ----------
 
 
 
  CASE WHEN Tool = 'bbd_missing' THEN COUNT(DISTINCT PK4special) ELSE 0 END AS bbdMissing,
  CASE WHEN Tool = 'bbd_added' THEN COUNT(PK) ELSE 0 END AS bbdAdded, -- nr de bbd-uri adaugate (4 bbd-uri adaugate => 4)
  CASE WHEN Tool = 'bbd_added' THEN COUNT(DISTINCT PK4special) ELSE 0 END AS taskAdded, -- nr de task-uri  (4 bbd-uri adaugate => 1)
  Is_added_directly,
  CASE WHEN Tool = 'bbd_added' AND Is_added_directly = 1 THEN COUNT(PK) ELSE 0 END AS bbdAddedDirectly,
  CASE WHEN Tool = 'bbd_added' AND Is_added_directly = 1 THEN COUNT(DISTINCT PK4special) ELSE 0 END AS taskAddedDirectly,
  CASE WHEN Tool = 'bbd_added' AND Is_added_directly = 0 THEN COUNT(DISTINCT PK4special) ELSE 0 END AS taskAddedByNotification,
 
  CASE WHEN Tool = 'bbd_to_be_checked' THEN COUNT(PK) ELSE 0 END AS bbdToBeChecked,
  CASE WHEN Tool = 'bbd_checked_not_found' OR Tool = 'bbd_checked_removed_from_shelf' THEN COUNT(PK) ELSE 0 END AS bbdChecked,
  CASE WHEN Tool = 'bbd_checked_not_found' THEN COUNT(PK) ELSE 0 END AS bbdCheckedNotFound,
  CASE WHEN Tool = 'bbd_checked_removed_from_shelf' THEN COUNT(PK) ELSE 0 END AS bbdCheckedRemovedFromShelf,
  CASE WHEN Tool = 'bbd_to_be_checked' AND Current_state = 'checked_not_found' THEN COUNT(PK) ELSE 0 END AS bbdTBCCheckedNotFound,
  CASE WHEN Tool = 'bbd_to_be_checked' AND Current_state = 'actioned' THEN COUNT(PK) ELSE 0 END AS bbdTBCActioned,
  CASE WHEN Tool LIKE 'bbd_actioned%' THEN COUNT(PK) ELSE 0 END AS bbdActioned, -- nr de bbd-uri actionate (4 actiuni pe un bbd => 4)
  CASE WHEN Tool LIKE '%bbd_actioned%' THEN COUNT(DISTINCT PK4special) ELSE 0 END AS taskActioned, -- nr de task-uri (4 actiuni => 1)
 
  CASE WHEN Tool = 'bbd_actioned_rotten' THEN COUNT(PK) ELSE 0 END AS nActionsRotten,
  CASE WHEN Tool = 'bbd_actioned_<unknown>' THEN COUNT(PK) ELSE 0 END AS nActionsunknown,
  CASE WHEN Tool = 'bbd_actioned_expirydate' THEN COUNT(PK) ELSE 0 END AS nActionsExpirydate,
  CASE WHEN Tool = 'bbd_actioned_donationanimalfood' THEN COUNT(PK) ELSE 0 END AS nActionsDonationanimalfood,
  CASE WHEN Tool = 'bbd_actioned_damaged' THEN COUNT(PK) ELSE 0 END AS nActionsDamaged,
  CASE WHEN Tool = 'bbd_actioned_extendedfoodbank' THEN COUNT(PK) ELSE 0 END AS nActionsExtendedfoodbank,
  CASE WHEN Tool = 'bbd_actioned_bbdexpired' THEN COUNT(PK) ELSE 0 END AS nActionsBbdexpired,
  CASE WHEN Tool = 'bbd_actioned_foodbank' THEN COUNT(PK) ELSE 0 END AS nActionsFoodbank,
  CASE WHEN Tool = 'bbd_actioned_donationanimalshelter' THEN COUNT(PK) ELSE 0 END AS nActionsDonationanimalshelter,
  CASE WHEN Tool = 'bbd_actioned_markdown' THEN COUNT(PK) ELSE 0 END AS nActionsMarkdown,
  CASE WHEN Tool = 'bbd_actioned_writeoff' THEN COUNT(PK) ELSE 0 END AS nActionsWriteoff,
 
 
  CASE WHEN Tool = 'bbd_actioned_rotten' THEN SUM(Quantity_manipulated) ELSE 0 END AS Quantity_manipulated_Rotten_SUM,
  CASE WHEN Tool = 'bbd_actioned_<unknown>' THEN SUM(Quantity_manipulated) ELSE 0 END AS Quantity_manipulated_unknown_SUM,
  CASE WHEN Tool = 'bbd_actioned_expirydate' THEN SUM(Quantity_manipulated) ELSE 0 END AS Quantity_manipulated_Expirydate_SUM,
  CASE WHEN Tool = 'bbd_actioned_donationanimalfood' THEN SUM(Quantity_manipulated) ELSE 0 END AS Quantity_manipulated_Donationanimalfood_SUM,
  CASE WHEN Tool = 'bbd_actioned_damaged' THEN SUM(Quantity_manipulated) ELSE 0 END AS Quantity_manipulated_Damaged_SUM,
  CASE WHEN Tool = 'bbd_actioned_extendedfoodbank' THEN SUM(Quantity_manipulated) ELSE 0 END AS Quantity_manipulated_Extendedfoodbank_SUM,
  CASE WHEN Tool = 'bbd_actioned_bbdexpired' THEN SUM(Quantity_manipulated) ELSE 0 END AS Quantity_manipulated_Bbdexpired_SUM,
  CASE WHEN Tool = 'bbd_actioned_foodbank' THEN SUM(Quantity_manipulated) ELSE 0 END AS Quantity_manipulated_Foodbank_SUM,
  CASE WHEN Tool = 'bbd_actioned_donationanimalshelter' THEN SUM(Quantity_manipulated) ELSE 0 END AS Quantity_manipulated_Donationanimalshelter_SUM,
  CASE WHEN Tool = 'bbd_actioned_markdown' THEN SUM(Quantity_manipulated) ELSE 0 END AS Quantity_manipulated_Markdown_SUM,
  CASE WHEN Tool = 'bbd_actioned_writeoff' THEN SUM(Quantity_manipulated) ELSE 0 END AS Quantity_manipulated_Writeoff_SUM,
  
 
  Reaction_time,
  SUM(Quantity_manipulated) AS Quantity_manipulated_SUM,
  CASE WHEN Tool = 'bbd_missing' THEN SUM(Quantity_manipulated) ELSE 0 END AS Quantity_manipulated_bbdMissing_SUM,
  CASE WHEN Tool = 'bbd_added' THEN SUM(Quantity_manipulated) ELSE 0 END AS Quantity_manipulated_bbdAdded_SUM,
  CASE WHEN Tool = 'bbd_to_be_checked' THEN SUM(Quantity_manipulated) ELSE 0 END AS Quantity_manipulated_bbdToBeChecked_SUM,
  CASE WHEN Tool = 'bbd_checked_not_found' THEN SUM(Quantity_manipulated) ELSE 0 END AS Quantity_manipulated_bbdCheckedNotFound_SUM,
  CASE WHEN Tool = 'bbd_checked_removed_from_shelf' THEN SUM(Quantity_manipulated) ELSE 0 END AS Quantity_manipulated_bbdCheckedRemovedFromShelf_SUM,
  CASE WHEN Tool = 'bbd_checked_not_found' OR Tool = 'bbd_checked_removed_from_shelf' THEN SUM(Quantity_manipulated) ELSE 0 END AS Quantity_manipulated_bbdChecked_SUM,
  CASE WHEN Tool LIKE 'bbd_actioned%' THEN SUM(Quantity_manipulated) ELSE 0 END AS Quantity_manipulated_bbdActioned_SUM,
  SUM(Available_stock) AS Available_stock_SUM,
  AVG(Grace_period) AS AVG_Grace_period,
 
  AVG(Days_to_execution_counter) AS AVG_Days_to_execution,
  CASE WHEN Tool = 'bbd_missing' THEN AVG(Days_to_execution_counter) ELSE 0 END AS Days_to_execution_bbdMissing_AVG,
  CASE WHEN Tool = 'bbd_added' THEN AVG(Days_to_execution_counter) ELSE 0 END AS Days_to_execution_bbdAdded_AVG,
  CASE WHEN Tool = 'bbd_to_be_checked' THEN AVG(Days_to_execution_counter) ELSE 0 END AS Days_to_execution_bbdToBeChecked_AVG,
  CASE WHEN Tool = 'bbd_checked_not_found' THEN AVG(Days_to_execution_counter) ELSE 0 END AS Days_to_execution_bbdCheckedNotFound_AVG,
  CASE WHEN Tool = 'bbd_checked_removed_from_shelf' THEN AVG(Days_to_execution_counter) ELSE 0 END AS Days_to_execution_bbdCheckedRemovedFromShelf_AVG,
  CASE WHEN Tool = 'bbd_checked_not_found' OR Tool = 'bbd_checked_removed_from_shelf' THEN AVG(Days_to_execution_counter) ELSE 0 END AS Days_to_execution_bbdChecked_AVG,
  CASE WHEN Tool LIKE 'bbd_actioned%' THEN AVG(Days_to_execution_counter) ELSE 0 END AS Days_to_execution_bbdActioned_AVG,
 
  AVG(Day_expiry_counter) AS AVG_Days_to_expiry,
  CASE WHEN Tool = 'bbd_missing' THEN AVG(Day_expiry_counter) ELSE 0 END AS Day_expiry_bbdMissing_AVG,
  CASE WHEN Tool = 'bbd_added' THEN AVG(Day_expiry_counter) ELSE 0 END AS Day_expiry_bbdAdded_AVG,
  CASE WHEN Tool = 'bbd_to_be_checked' THEN AVG(Day_expiry_counter) ELSE 0 END AS Day_expiry_bbdToBeChecked_AVG,
  CASE WHEN Tool = 'bbd_checked_not_found' THEN AVG(Day_expiry_counter) ELSE 0 END AS Day_expiry_checkedNotFound_AVG,
  CASE WHEN Tool = 'bbd_checked_not_found' OR Tool = 'bbd_checked_removed_from_shelf' THEN AVG(Day_expiry_counter) ELSE 0 END AS Day_expiry_bbdChecked_AVG,
  CASE WHEN Tool = 'bbd_checked_not_found' THEN AVG(Day_expiry_counter) ELSE 0 END AS Day_expiry_bbdCheckedNotFound_AVG,
  CASE WHEN Tool = 'bbd_checked_removed_from_shelf' THEN AVG(Day_expiry_counter) ELSE 0 END AS Day_expiry_bbdCheckedRemovedFromShelf_AVG,
  CASE WHEN Tool LIKE 'bbd_actioned%' THEN AVG(Day_expiry_counter) ELSE 0 END AS Day_expiry_bbdActioned_AVG,
FROM (
  SELECT * EXCEPT (Summ, Change, Previous_current_state, Grace_period, current_state_new), Grace_period
  FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_NL\`
  UNION ALL
  SELECT * EXCEPT (Grace_period), Grace_period
  FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_daily_NL\`

)
GROUP BY Store_name, Country, Date, Store_no, Tool, Tool_new, Department_name, Department_no, PMMG, PMG, PMSG, User_type, 
state_of_checked, current_state,
-- current_state_new, 
Reaction_time, Is_added_directly
)
UNION ALL
(SELECT
  Store_name, Country, Date, Store_no, Tool, Tool_new, Department_name, Department_no, PMMG, PMG, PMSG, User_type,
  ...
  CASE WHEN Tool LIKE 'bbd_actioned%' THEN AVG(Day_expiry_counter) ELSE 0 END AS Day_expiry_bbdActioned_AVG,
FROM (
  -- SELECT * EXCEPT (Summ, Change, User_type, Creation_user), Summ, Change, User_type, Creation_user
  -- FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_RS\`
  -- UNION ALL
  -- SELECT * EXCEPT (Summ, Change, User_type, Creation_user), Summ, Change, User_type, Creation_user
  -- FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_daily_RS\`
  SELECT * EXCEPT (Summ, Change, Previous_current_state, Grace_period, current_state_new), Grace_period
  FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_RS\`
  UNION ALL
  SELECT * EXCEPT (Grace_period, Syst_stock_end_of_day, Mado_stock_end_of_day, Available_stock_end_of_day), Grace_period
  FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_daily_RS\`
)
GROUP BY Store_name, Country, Date, Store_no, Tool, Tool_new, Department_name, Department_no, PMMG, PMG, PMSG, User_type, 
state_of_checked, current_state,
...
FROM (
  -- SELECT * EXCEPT (Summ, Change, User_type, Creation_user), Summ, Change, User_type, Creation_user
  -- FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_IT\`
  -- UNION ALL
  -- SELECT * EXCEPT (Summ, Change, User_type, Creation_user), Summ, Change, User_type, Creation_user
  -- FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_daily_IT\`
  SELECT * EXCEPT (Summ, Change, Previous_current_state, Grace_period, current_state_new), Grace_period
  FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_IT\`
  UNION ALL
  SELECT * EXCEPT (Grace_period, Syst_stock_end_of_day, Mado_stock_end_of_day, Available_stock_end_of_day), Grace_period
...
FROM (
  SELECT * EXCEPT (Summ, Change, Previous_current_state, Grace_period, current_state_new), Grace_period
  FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_PL\`
  UNION ALL
  SELECT * EXCEPT (Grace_period, Syst_stock_end_of_day, Mado_stock_end_of_day, Available_stock_end_of_day), Grace_period
  FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_daily_PL\`
)
GROUP BY Store_name, Country, Date, Store_no, Tool, Tool_new, Department_name, Department_no, PMMG, PMG, PMSG, User_type, 
state_of_checked, current_state,
-- current_state_new, 
...
FROM (
  -- SELECT * EXCEPT (Summ, Change, User_type, Creation_user), Summ, Change, User_type, Creation_user
  -- FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_HU\`
  -- UNION ALL
  -- SELECT * EXCEPT (Summ, Change, User_type, Creation_user), Summ, Change, User_type, Creation_user
  -- FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_daily_HU\`
  SELECT * EXCEPT (Summ, Change, Previous_current_state, Grace_period, current_state_new), Grace_period
  FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_HU\`
  UNION ALL
  SELECT * EXCEPT (Grace_period, Syst_stock_end_of_day, Mado_stock_end_of_day, Available_stock_end_of_day), Grace_period
...
(SELECT
  ...
  CASE WHEN Tool LIKE 'bbd_actioned%' THEN AVG(Day_expiry_counter) ELSE 0 END AS Day_expiry_bbdActioned_AVG,
FROM (
  -- SELECT * EXCEPT (Summ, Change, User_type, Creation_user, Grace_period), Summ, Change, User_type, Creation_user, Grace_period
  -- FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_TR\`
  -- UNION ALL
  -- SELECT * EXCEPT (Summ, Change, User_type, Creation_user, Grace_period), Summ, Change, User_type, Creation_user, Grace_period
  -- FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_daily_TR\`
  SELECT * EXCEPT (Summ, Change, Previous_current_state, Grace_period, current_state_new), Grace_period
  FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_TR\`
  UNION ALL
  SELECT * EXCEPT (Grace_period, Syst_stock_end_of_day, Mado_stock_end_of_day, Available_stock_end_of_day), Grace_period
...
FROM (
  -- SELECT * EXCEPT (Summ, Change, User_type, Creation_user), Summ, Change, User_type, Creation_user
  -- FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_SK\`
  -- UNION ALL
  -- SELECT * EXCEPT (Summ, Change, User_type, Creation_user), Summ, Change, User_type, Creation_user
  -- FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_daily_SK\`
  SELECT * EXCEPT (Summ, Change, Previous_current_state, Grace_period, current_state_new), Grace_period
  FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_SK\`
  UNION ALL
  SELECT * EXCEPT (Grace_period, Syst_stock_end_of_day, Mado_stock_end_of_day, Available_stock_end_of_day), Grace_period
...
(SELECT
  ...
  CASE WHEN Tool LIKE 'bbd_actioned%' THEN AVG(Day_expiry_counter) ELSE 0 END AS Day_expiry_bbdActioned_AVG,
FROM (
  -- SELECT * FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_DE_3\`
  -- UNION ALL
  -- SELECT * FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_DE_daily_3\`
  SELECT * EXCEPT (Summ, Change, Previous_current_state, Grace_period, current_state_new), Grace_period
  FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_DE_3\`
  UNION ALL
  SELECT * EXCEPT (Grace_period, Syst_stock_end_of_day, Mado_stock_end_of_day, Available_stock_end_of_day), Grace_period
  FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_DE_daily_3\`
)
GROUP BY Store_name, Country, Date, Store_no, Tool, Tool_new, Department_name, Department_no, PMMG, PMG, PMSG, User_type, 
state_of_checked, current_state,
-- current_state_new, 
...
(SELECT
  ...
  CASE WHEN Tool LIKE 'bbd_actioned%' THEN AVG(Day_expiry_counter) ELSE 0 END AS Day_expiry_bbdActioned_AVG,
FROM (
  -- SELECT DISTINCT * EXCEPT (Summ, Change, Previous_current_state, current_state_new, Creation_user, User_type), Creation_user, User_type
  -- -- , Summ, Change, User_type, Creation_user
  -- FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_ES\`
  -- UNION ALL
  -- SELECT DISTINCT * EXCEPT (Creation_user, User_type), Creation_user, User_type
  -- -- EXCEPT (Summ, Change, User_type, Creation_user), Summ, Change, User_type, Creation_user
  -- FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_daily_ES\`
  SELECT * EXCEPT (Summ, Change, Previous_current_state, Grace_period, current_state_new), Grace_period
  FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_ES\`
  UNION ALL
  SELECT * EXCEPT (Grace_period, Syst_stock_end_of_day, Mado_stock_end_of_day, Available_stock_end_of_day), Grace_period
...
FROM (
  -- SELECT * EXCEPT (Summ, Change, User_type, Creation_user), Summ, Change, User_type, Creation_user
  -- FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_PK\`
  -- UNION ALL
  -- SELECT * EXCEPT (Summ, Change, User_type, Creation_user), Summ, Change, User_type, Creation_user
  -- FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_daily_PK\`
  SELECT * EXCEPT (Summ, Change, Previous_current_state, Grace_period, current_state_new), Grace_period
  FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_PK\`
  UNION ALL
  SELECT * EXCEPT (Grace_period, Syst_stock_end_of_day, Mado_stock_end_of_day, Available_stock_end_of_day), Grace_period
...
FROM (
  -- SELECT * EXCEPT (Summ, Change, User_type, Creation_user), Summ, Change, User_type, Creation_user
  -- FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_BG\`
  -- UNION ALL
  -- SELECT * EXCEPT (Summ, Change, User_type, Creation_user), Summ, Change, User_type, Creation_user
  -- FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_daily_BG\`
  SELECT * EXCEPT (Summ, Change, Previous_current_state, Grace_period, current_state_new), Grace_period
  FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_BG\`
  UNION ALL
  SELECT * EXCEPT (Grace_period, Syst_stock_end_of_day, Mado_stock_end_of_day, Available_stock_end_of_day), Grace_period
...
FROM (
  -- SELECT * EXCEPT (Summ, Change, User_type, Creation_user), Summ, Change, User_type, Creation_user
  -- FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_KZ\`
  -- UNION ALL
  -- SELECT * EXCEPT (Summ, Change, User_type, Creation_user), Summ, Change, User_type, Creation_user
  -- FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_daily_KZ\`
  SELECT * EXCEPT (Summ, Change, Previous_current_state, Grace_period, current_state_new), Grace_period
  FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_KZ\`
  UNION ALL
  SELECT * EXCEPT (Grace_period, Syst_stock_end_of_day, Mado_stock_end_of_day, Available_stock_end_of_day), Grace_period
...
FROM (
  -- SELECT * EXCEPT (Summ, Change, User_type, Creation_user), Summ, Change, User_type, Creation_user
  -- FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_MD\`
  -- UNION ALL
  -- SELECT * EXCEPT (Summ, Change, User_type, Creation_user), Summ, Change, User_type, Creation_user
  -- FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_daily_MD\`
  SELECT * EXCEPT (Summ, Change, Previous_current_state, Grace_period, current_state_new), Grace_period
  FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_MD\`
  UNION ALL
  SELECT * EXCEPT (Grace_period, Syst_stock_end_of_day, Mado_stock_end_of_day, Available_stock_end_of_day), Grace_period
...
FROM (
  -- SELECT DISTINCT *
  -- EXCEPT (Summ, Change, Previous_current_state, current_state_new)
  -- FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_UA\`
  -- UNION ALL
  -- SELECT * FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_daily_UA\`
  SELECT * EXCEPT (Summ, Change, Previous_current_state, Grace_period, current_state_new), Grace_period
  FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_UA\`
  UNION ALL
  SELECT * EXCEPT (Grace_period, Syst_stock_end_of_day, Mado_stock_end_of_day, Available_stock_end_of_day), Grace_period
  FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_daily_UA\`
)
GROUP BY Store_name, Country, Date, Store_no, Tool, Tool_new, Department_name, Department_no, PMMG, PMG, PMSG, User_type, 
state_of_checked, current_state,
-- current_state_new, 
...
FROM (
  -- SELECT * EXCEPT (Summ, Change, User_type, Creation_user), Summ, Change, User_type, Creation_user
  -- FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_PT\`
  -- UNION ALL
  -- SELECT * EXCEPT (Summ, Change, User_type, Creation_user), Summ, Change, User_type, Creation_user
  -- FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_daily_PT\`
  SELECT * EXCEPT (Summ, Change, Previous_current_state, Grace_period, current_state_new), Grace_period
  FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_PT\`
  UNION ALL
  SELECT * EXCEPT (Grace_period, Syst_stock_end_of_day, Mado_stock_end_of_day, Available_stock_end_of_day), Grace_period
...
FROM (
  -- SELECT * EXCEPT (Summ, Change, User_type, Creation_user), Summ, Change, User_type, Creation_user
  -- FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_HR\`
  -- UNION ALL
  -- SELECT * EXCEPT (Summ, Change, User_type, Creation_user), Summ, Change, User_type, Creation_user
  -- FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_daily_HR\`
  SELECT * EXCEPT (Summ, Change, Previous_current_state, Grace_period, current_state_new), Grace_period
  FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_HR\`
  UNION ALL
  SELECT * EXCEPT (Grace_period, Syst_stock_end_of_day, Mado_stock_end_of_day, Available_stock_end_of_day), Grace_period
...
--   SELECT * EXCEPT (Grace_period, Syst_stock_end_of_day, Mado_stock_end_of_day, Available_stock_end_of_day), Grace_period
--   -- SELECT * EXCEPT (Grace_period), Grace_period
--   FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_daily_RU\`
-- )
-- GROUP BY Store_name, Country, Date, Store_no, Tool, Tool_new, Department_name, Department_no, PMMG, PMG, PMSG, User_type, 
-- state_of_checked, current_state,
-- -- current_state_new, 
...
FROM (
  -- SELECT * EXCEPT (Summ, Change, User_type, Creation_user), Summ, Change, User_type, Creation_user
  -- FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_CZ\`
  -- UNION ALL
  -- SELECT * EXCEPT (Summ, Change, User_type, Creation_user), Summ, Change, User_type, Creation_user
  -- FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_daily_CZ\`
  SELECT * EXCEPT (Summ, Change, Previous_current_state, Grace_period, current_state_new), Grace_period
  FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_CZ\`
  UNION ALL
  SELECT * EXCEPT (Grace_period, Syst_stock_end_of_day, Mado_stock_end_of_day, Available_stock_end_of_day), Grace_period
...
FROM (
  -- SELECT * EXCEPT (Summ, Change, User_type, Creation_user), Summ, Change, User_type, Creation_user
  -- FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_FR\`
  -- UNION ALL
  -- SELECT * EXCEPT (Summ, Change, User_type, Creation_user), Summ, Change, User_type, Creation_user
  -- FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_daily_FR\`
  -- -- SELECT * EXCEPT (Summ, Change, User_type, Creation_user), Summ, Change, User_type, Creation_user
  -- -- FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_FR_daily_new\` 
  SELECT * EXCEPT (Summ, Change, Previous_current_state, Grace_period, current_state_new), Grace_period
  FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_FR\`
  UNION ALL
  SELECT * EXCEPT (Grace_period, Syst_stock_end_of_day, Mado_stock_end_of_day, Available_stock_end_of_day), Grace_period
...
FROM (
  -- SELECT * EXCEPT (Summ, Change, User_type, Creation_user), Summ, Change, User_type, Creation_user
  -- FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_RO\`
  -- UNION ALL
  -- SELECT * EXCEPT (Summ, Change, User_type, Creation_user), Summ, Change, User_type, Creation_user
  -- FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_daily_RO\`
  SELECT * EXCEPT (Summ, Change, Previous_current_state, Grace_period, current_state_new), Grace_period
  FROM \`metro-bi-wb-inventory-s00.Country_dashboards.bbd_operational2_RO\`
  UNION ALL
  SELECT * EXCEPT (Grace_period, Syst_stock_end_of_day, Mado_stock_end_of_day, Available_stock_end_of_day), Grace_period
...
WHERE
  ((User_type IS NULL) OR (User_type NOT IN ('BBD_RULE_REFRESH', 'BBDExpired', 'BBDStockZero')))
  AND ((Reaction_time IS NULL) OR (Reaction_time != 'Zero stock-no action'))
  -- AND Date <= (CURRENT_DATE - 1)
  -- AND Department_name IS NOT NULL
  -- AND Tool IN ('bbd_missing', 'bbd_added', 'bbd_to_be_checked', 'bbd_checked_not_found', 'bbd_checked_removed_from_shelf')
  -- AND (Tool_new IS NULL OR Tool_new != 'bbd_checked_removed_from_shelf_pending')
...
module.exports = { projectFor, bbd_operational0_XX, bbd_operational1_XX, bbd_operational2_1_XX, bbd_operational2_XX, bbd_usage};